...
int wyn_main() {
    const int negative = -15;
    ;
    const int int_abs = abs_val(negative);
    ;
    const int even_num = 8;
    ;
    const int is_even_check = is_even(even_num);
    ;
    const double pi = 3.14;
    ;
    const int floor_val = floor(pi);
    ;
    const int ceil_val = ceil(pi);
    ;
    const int round_val = round(pi);
    ;
    const bool flag = true;
    ;
    const int bool_as_int = (int)(flag);
    ;
    const WynArray numbers = ({ WynArray __arr_0 = array_new(); array_push_int(&__arr_0, 5); array_push_int(&__arr_0, 10); array_push_int(&__arr_0, 15); array_push_int(&__arr_0, 20); __arr_0; });
    ;
    const int array_sum = arr_sum(numbers, (numbers).count);
    ;
    const int array_max = arr_max(numbers, (numbers).count);
    ;
    const int array_min = arr_min(numbers, (numbers).count);
    ;
    const const char* text = "Hello";
    ;
    const int str_length = string_length(text);
    ;
    return ((((((((((int_abs + is_even_check) + floor_val) + ceil_val) + round_val) + bool_as_int) + array_sum) + array_max) + array_min) + str_length) - 7);
}
