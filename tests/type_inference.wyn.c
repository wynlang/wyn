...
typedef enum { WYN_TYPE_INT, WYN_TYPE_FLOAT, WYN_TYPE_STRING, WYN_TYPE_ARRAY } WynValueType;
typedef struct {
    WynValueType type;
    union {
        int int_val;
...
void print_error(WynError err) { printf("%s: %s", err.type, err.message); }
...
WynObject* wyn_arc_alloc(size_t size) {
...
int test_variable_inference();
int test_function_return_inference();
int test_complex_inference();
int main();
