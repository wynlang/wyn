...
# A MUTATION-FLAG RACE DETECTOR FIRES PROBABILISTICALLY, so asserting it on a
# single run is a flaky gate - and this was one. The HashSet case failed on a
# macOS CI runner (printing its result instead of panicking) because the two tasks
# happened to serialise: the flag only trips when two mutations genuinely overlap,
# and nothing forces them to. Go's map race detector has the same property.
#
...
# them. Each attempt is a fresh process, so attempts are independent.
#
# A wrong-but-quiet run is also reported, because that is the interesting case: it
# means the tasks overlapped, corrupted nothing, and produced a value anyway - which
# is precisely the "lucky, not safe" state this barrier exists to end.
