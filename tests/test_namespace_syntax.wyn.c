...
typedef struct {
    enum { Result_Ok_TAG, Result_Err_TAG } tag;
    union {
        int Ok_value;
...
Result Result_Ok(int value) {
    Result result;
    result.tag = Result_Ok_TAG;
    result.data.Ok_value = value;
    return result;
}

Result Result_Err(const char* value) {
    Result result;
    result.tag = Result_Err_TAG;
    result.data.Err_value = value;
    return result;
}

const char* Result_toString(Result val) {
    switch(val.tag) {
        case Result_Ok_TAG: return "Ok";
        case Result_Err_TAG: return "Err";
    }
    return "Unknown";
}

int Result_unwrap(Result val) {
    if (val.tag == Result_Ok_TAG) {
        return val.data.Ok_value;
    }
    fprintf(stderr, "Error: unwrap() called on Err\n");
    exit(1);
}

bool Result_is_ok(Result val) {
    return val.tag == Result_Ok_TAG;
}

bool Result_is_err(Result val) {
    return val.tag == Result_Err_TAG;
}

int Result_unwrap_or(Result val, int default_val) {
    if (val.tag == Result_Ok_TAG) {
        return val.data.Ok_value;
    }
    return default_val;
...
int wyn_main() {
    __auto_type x = Result_Ok(42);
    ;
    {
    __auto_type __match_val = x;
    if (__match_val.tag == Result_Ok_TAG) {
        __auto_type n = __match_val.data.Ok_value;
            print(n);
    return n;
    } else if (__match_val.tag == Result_Err_TAG) {
        __auto_type e = __match_val.data.Err_value;
            print("Error");
    return 0;
    }
}
