    ...
    switch (expr->type) {
        ...
        case EXPR_ASSIGN: {
            ...
            if (val_type && sym->type) {
                ...
                // The `!= TYPE_STRUCT` clause below means a STRUCT-typed target was
                // exempt from ALL assignment checking, so once a variable held a
                // struct you could assign anything to it and only the C compiler
                // objected:
                //
                //   m = A { n: 1 }
                ...
                // value) was already rejected, and same-struct assignment is fine,
                // so the hole was exactly this direction. Found by the fuzzer at
                // three separate seeds.
                //
                // The exemption is kept for everything it plausibly protected: the
                // check below fires only when the VALUE is a plain scalar, or when
                // both sides are named user structs with DIFFERENT names. The
                // monomorphic Option*/Result* families are TYPE_STRUCT too and are
                // reassigned across family names legitimately (OptionInt vs
                // OptionUser), so they are excluded rather than risk rejecting
                // correct code - which would be worse than the hole.
