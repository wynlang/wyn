...
int wyn_main() {
    const WynArray numbers = ({ WynArray __arr_0 = array_new(); array_push_int(&__arr_0, 5); array_push_int(&__arr_0, 10); array_push_int(&__arr_0, 15); array_push_int(&__arr_0, 20); array_push_int(&__arr_0, 25); __arr_0; });
    ;
    ...
    ;
    const const char* text = "Wyn";
    ;
    const int text_len = string_length(text);
    ;
    const int negative = -10;
    ;
    const int absolute = abs_val(negative);
    ;
    const int num = 6;
    ;
    const int even_check = is_even(num);
    ;
    return ((((((array_sum + array_max) + array_min) + text_len) + absolute) + even_check) - 19);
}
