...
int create_new_project_with_template(const char* name, const char* template, const char* lib_target) {
    ...
    if (strcmp(template, "web") == 0) {
        ...
        fprintf(f,
            ...
            "    // Database CRUD\n"
            "    var db = Db.open(\"/tmp/%s_test.db\")\n"
            "    // DROP (not DELETE): the db file persists across runs and DELETE\n"
            "    // doesn't reset AUTOINCREMENT, so id-based asserts would fail on re-run\n"
            "    Db.exec(db, \"DROP TABLE IF EXISTS items\")\n"
            "    Db.exec(db, \"CREATE TABLE items (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, created_at TEXT DEFAULT CURRENT_TIMESTAMP)\")\n\n"
            "    // Create\n"
